...
import { MeterReadingFormWithOCR } from "../../meter-reading/components/MeterReadingFormWithOCR";
...
export function InvoiceFormDialog({ open, onClose, onSuccess, editInvoice }) {
  ...
  useEffect(() => {
    ...
    if (editInvoice) {
      ...
      setElectricPrice(
        editInvoice.electricPrice || DEFAULT_PRICES.electricPrice,
      );
      ...
      setLoadingAuto(true);
      InvoiceService.getLatestMeterReading(
        editInvoice.roomId,
        editInvoice.contractId,
      )
        .then((res) => {
          if (res.success && res.data) {
            if (res.data.month === editInvoice.month) {
              ...
              setElectricNew(
                String(
                  (res.data.electricNew ?? 0) +
                    (editInvoice.electricUsage || 0),
                ),
              );
              setWaterNew(
                String(
                  (res.data.waterNew ?? 0) + (editInvoice.waterUsage || 0),
                ),
              );
            }
          } else {
        ...
        .then(([contractRes, invoiceRes]) => {
          if (contractRes.success && invoiceRes.success) {
            ...
            const invoicedRoomIds = new Set(
              invoicesCurrentMonth.map((inv) => inv.roomId),
            );
  ...
  const handleSelectContract = useCallback(async (contract) => {
    ...
    try {
      ...
      if (priceRes.success && priceRes.data) {
        setElectricPrice(
          priceRes.data.electricPrice ?? DEFAULT_PRICES.electricPrice,
        );
  ...
  const electricUsage = Math.max(
    0,
    (Number.parseFloat(electricNew) || 0) - electricOld,
  );
  const waterUsage = Math.max(0, (Number.parseFloat(waterNew) || 0) - waterOld);
  ...
  // Validation
  const electricNewNum = Number.parseFloat(electricNew);
  const waterNewNum = Number.parseFloat(waterNew);
              ...
              <div>
                <p className="text-sm text-white/80">
                  {editInvoice
                    ...
                    : step === 1
                      ? "Bước 1/2"
                      : `Bước 2/2 — Phòng ${selectedContract?.roomCode}`}
                </p>
                <h2 className="text-xl font-semibold">
                  {editInvoice
                    ? `Phòng ${editInvoice.roomCode}`
                    : step === 1
                      ? "Chọn phòng"
                      : "Nhập chỉ số & phí"}
                </h2>
                <p className="text-sm text-white/70 mt-0.5">
                  Tháng {editInvoice ? editInvoice.month : currentMonth}
                </p>
              </div>
              <button
                onClick={handleClose}
                ...
                {loadingContracts ? (
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    <p className="text-sm mt-1">
                      Tất cả các phòng (đang có khách) đều đã được tạo hóa đơn
                      trong tháng này.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-indigo-600 font-medium mt-0.5">
                              {(contract.currentPrice || 0).toLocaleString(
                                "vi-VN",
                              )}{" "}
                              ₫/tháng
                            </p>
                ...
                <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                  ...
                  <div className="flex gap-2 mb-2">
                    <TextField
                      ...
                      error={electricError}
                      helperText={
                        electricError ? "Chỉ số mới phải ≥ chỉ số cũ" : ""
                      }
                      sx={{
                ...
                <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                  ...
                  <div className="flex gap-2 mb-2">
                    <TextField
                      ...
                      error={waterError}
                      helperText={
                        waterError ? "Chỉ số mới phải ≥ chỉ số cũ" : ""
                      }
                      sx={{
                        "& .MuiOutlinedInput-root": { borderRadius: "12px" },
                      }}
                ...
                <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <TextField
                      ...
                      value={internetFee}
                      onChange={(e) =>
                        setInternetFee(Number(e.target.value) || 0)
                      }
                      size="small"
                      InputProps={{
                        startAdornment: (
                          <Wifi size={14} className="mr-1 text-gray-400" />
                        ),
                      }}
                      sx={{
                        "& .MuiOutlinedInput-root": { borderRadius: "12px" },
                    ...
                    <TextField
                      ...
                      value={serviceFee}
                      onChange={(e) =>
                        setServiceFee(Number(e.target.value) || 0)
                      }
                      size="small"
                      sx={{
                        "& .MuiOutlinedInput-root": { borderRadius: "12px" },
        ...
        <div className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 min-h-screen">
          {meterDialogType && selectedContract && (
            <MeterReadingFormWithOCR
              ...
              previousReading={
                meterDialogType === "electric" ? electricOld : waterOld
              }
              month={editInvoice ? editInvoice.month : currentMonth}
              userId={selectedContract.representativeTenantId || "system"}
              onConfirm={(data) => {
                // data contains: reading, imageUrl, confidence, etc.
                if (meterDialogType === "electric") {
                  setElectricNew(String(data.reading));
                } else {
                  setWaterNew(String(data.reading));
                }
